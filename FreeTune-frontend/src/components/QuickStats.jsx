...
const QuickStats = () => {
  ...
  const calculateMusicStats = () => {
    const totalSongs = allMusicData.length;
    let totalDuration = 0;
    allMusicData.forEach((music) => {
      
      totalDuration += music.duration;
    });
    let sharedSong = allMusicData.filter((music) => music.isShared === true);
    ...
    setMusicStats({
      ...
      totalDuration:
        totalDuration < 3600
          ? totalDuration === 0
            ? 0
            : `${Math.floor(totalDuration / 60)}:${Math.floor(totalDuration % 60)}m`
          : `${Math.floor(totalDuration / 3600)}:${Math.floor(totalDuration % 3600)}h`,
